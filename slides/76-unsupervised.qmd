...
We call `as_tibble()` after `scale()` because the scale function returns a matrix-like object.
...
We can see that now the two variables (`ir` and `green` reflectance) are centered at zero and in roughly in range (-2, 2). The scale of the variables ahas changed, but the main pattern in the data is unchanged.

Let's use the function [prcomp()]() to run a principant component analysis on this dataset.
...
We have used the function `prcomp()` and stored its output in `pca`. The ouptut of `prcomp()` is a list with 5 elements in it.
...
Let's give an interpretation of the PCA results from an explorative-data-analysis point of view.

### Analitical intepretation of PCA results
