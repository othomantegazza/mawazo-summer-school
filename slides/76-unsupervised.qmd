...
We have the projection of the `IR` and `green` variable on the two principal components is stored in the matrix `x` stored within the list `pca` . Let's access this object calling `pca$x`, and turn it into a tibble to explore it's content.
...
By definition the principal components are ordered by the standard deviation of the data projected on them, in decreasing order. Let's check it:
