...
R has adopted the tilde model formula definition that we have seen above to define models within it's function. So, for example, the formula: `spikelet_number_sp_n ~ primary_branch_number_pb` will use:
...
The [`summary()` method](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/summary.lm) prints the coefficients that we've seen above, together with statistics measuring their relevance, and scores of the overall model.

```{r}
fit %>% summary()
```

To access those coefficients and statistics systematically, what better choice than to tidy them in a tibble with the package [broom](https://broom.tidymodels.org/)
...
`tidy()` summarizes information about model components in a tibble.
